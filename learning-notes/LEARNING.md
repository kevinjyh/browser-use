...
- 我的背景:
  我是一名自學的業餘 Python 程式設計愛好者，我從未與其他人共同開發 Python 專案，我只寫過500行以下的幾個小型 Python 程式供自己使用的經歷。

- 我對本專案的學習目標: 理解底層邏輯及本專案的使用方法

- 我希望你以專業 Python 程式設計師的角色，幫我對本專案全部源碼及相關內容完成以下的工作:

1. 請先梳理這個專案的所有功能，各個函式的交互邏輯，讓一個非技術人員的用戶能理解這個專案。
2. 請你建議我對本專案的整體研究路線圖及時間期程表，以達成我上面所述的學習目標。
3. 依照我上面的需求，由淺入深的順序列出符合我學習目標的代碼檔案。
4. 閱讀 `tests\` 目錄下所有的測試案例後，列舉我使用這些測試案例時，由淺入深的順序是什麼。

- 完成以上工作後，請將你所有的回應內容寫入 `本專案根目錄\learning-notes\LEARNING.md` 檔案中。
